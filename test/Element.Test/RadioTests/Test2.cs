            ...
            for (int i = 0; i < radios.Length; i++)
            {
                ...
                if (i == checkedIndex)
                {
                    if (i == 0)
                    {
                        if (isInit)
                        {
                            Assert.Equal("el-radio is-checked is-disabled  el-radio-button--default", cls);
                            el = await radio.QuerySelectorAsync("span.el-radio__input.is-checked > span.el-radio__inner");
                        }
                        else
                        {
                            Assert.Equal("el-radio  is-disabled  el-radio-button--default", cls);
                            el = await radio.QuerySelectorAsync("span.el-radio__input.is-disabled > span.el-radio__inner");
                        }
                    }
                    else
                    {
